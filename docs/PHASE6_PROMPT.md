...
- **Phase 5 완료**: docs/PHASE5.md 참고. 5개 Step + 후속 수정 3건 전부 구현·검증 완료. 225개 테스트 PASSED. jpackage 포터블 .exe 빌드 완료.
...
- **기존 인프라**: GitExecutor, GitService(clone, resetSoft 포함), GitHubService, RepositoryManager, TaskManager(비동기), EventBus, ConfigManager(원자적 저장), TokenManager(PAT 저장/로드), SettingsController, ErrorMessages. WorkflowService는 DESIGN.md에 명시되어 있으나 아직 미구현.

## Phase 6 범위 (DESIGN.md §Phase 6 — 선택적 확장)
1. **(보류) 원클릭 워크플로우** — Commit + Push 한 번에. 실사용 시 필요성 낮아 보류.
2. **시스템 트레이** — 앱 최소화 → 트레이 상주, 알림
3. **커밋 메시지 히스토리** — 최근 메시지 드롭다운
4. **파일 필터** — 변경 파일 목록 검색 (이미 구현: unstagedSearchField/stagedSearchField. 검증만 필요)
...
## 참고할 기존 코드
- `controller/MainController.java` — 현재 ~2100줄
- `controller/SettingsController.java` — 설정 다이얼로그 (테마, 자동 Fetch 주기, Clone 경로, 외부 터미널, PAT)
- `controller/component/DiffRenderer.java`, `FileChangeListCell.java`, `RepoListCell.java` — UI 컴포넌트
...
- `config/AppConfig.java` — 설정 모델 (repoPaths, theme, autoFetchIntervalMinutes, windowWidth/Height/X/Y, defaultClonePath, externalTerminal)
- `config/ConfigManager.java` — JSON 설정 읽기/쓰기 (원자적 저장)
- `GitMiniApp.java` — 앱 진입점, 서비스 생성·주입, 테마 적용, 창 상태 저장/복원 (화면 범위 클램핑 포함)
- `Launcher.java` — jpackage용 진입점 (non-modular JavaFX)
...
- **참고**: 실제 구현은 DESIGN.md 패키지 구조와 일부 다름 (RepoDetailController 미분리, RepoCardController → RepoListCell)
